...
static int gve_transmit_dqo ( struct net_device *netdev,
	...
	if ( gve_is_qpl(gve) ) {
		for ( offset = 0 ; offset < len ; offset += frag_len ) {
			...
			desc->buf_size = frag_len; // TODO: check after converting to le16
...
static void gve_poll_rx_dqo ( struct net_device *netdev ) {
	...
	while ( 1 ) {
		...
		/* Check generation bit */
		// DBGC( gve, "GVE %p RX poll %#04x/%#04x index %#04x gen %#04x/%#04x\n",
		// 	gve, rx->cons, rx->prod, index,
		// 	cmplt->generation, rx->cur_gen_bit );
		usleep(10);
		rmb();
		...
		/* Allocate and populate I/O buffer */
		if ( cmplt->rx_error == 0 ) {
			struct io_buffer *iobuf = alloc_iob ( len );
			if ( ! iobuf ) {
				rc = -ENOMEM;
				netdev_rx_err ( netdev, NULL, rc );
